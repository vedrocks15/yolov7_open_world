...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        # setting attributes of the detector head....
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    # Creating soft label targets (scope to introduce distillation vector targets...)
                    t = torch.full_like(ps[:, 5:], self.cn, device=device)  # targets
                    t[range(n), tcls[i]] = self.cp
                    #t[t==self.cp] = iou.detach().clamp(0).type(t.dtype)
                    
                    # Add distillation loss here.....
...
class ComputeLoss_CLIP:
    ...
    def __init__(self, 
        ...
        # setting attributes of the detector head.... (extracting head properties from the model)
        for k in 'na', 'nc', 'nl', 'anchors':
            setattr(self, k, getattr(det, k))
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            n = b.shape[0]  # number of targets
            if n:
                # extracting mapped prediction-targets...
                ps = pi[b, a, gj, gi]  # prediction subset corresponding to targets
                ...
                # Open world CLIP vector classification....

                # extracting semantic predictions of 512 dims for clip distillation.... (positive preds)
    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            if nt:
                # Matches
                r = t[:, :, 4:6] / anchors[:, None]  # wh ratio
                j = torch.max(r, 1. / r).max(2)[0] < self.hyp['anchor_t']  # compare (true false list)
                # j = wh_iou(anchors, t[:, 4:6]) > model.hyp['iou_t']  # iou(3,n)=wh_iou(anchors(3,2), gwh(n,2))
                
                # set of positive anchors within threshold....
                t = t[j]  # filter
...
class ComputeLossOTA_CLIP:
    ...
    def __init__(self, 
        ...
        super(ComputeLossOTA_CLIP, self).__init__()

        device = next(model.parameters()).device  # get model device
        h = model.hyp  # hyperparameters

        # Define criteria
        BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['cls_pw']], device=device)) # Replace with l2 loss
        ...
        # Extracting the appropriate model head
        det = model.module.model[-1] if is_parallel(model) else model.model[-1]  # Detect() module
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # <<<<<<<<<<<CLIP DISTILLATION TO BE ADDED HERE>>>>>>>>>>
                # Classification
                selected_tcls = targets[i][:, 1].long()
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t = torch.full_like(ps[:, 5:], self.cn, device=device)  # targets
